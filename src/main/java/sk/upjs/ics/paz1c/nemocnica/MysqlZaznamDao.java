...
public class MysqlZaznamDao implements ZaznamDAO {
    ...
    public List<Zaznam> dajZaznamy() {
        String sql = "SELECT lekar_id,pacient_id,diagnoza_id,liek_id FROM zaznam";
    ...
    public void pridajZaznam(Zaznam zaznam) { 
        if((zaznam.getLekar()==null) || 
            (zaznam.getPacient()==null) || 
            (zaznam.getDiagnoza()==null) || 
            (zaznam.getLiek()==null)){
            return;
        }
        
        // ak je entita nova tak, tak jej priradi nove id a vlozi ho na koniec 
        if(zaznam.getLekar().getId()==0){
             zaznam.getLekar().setId(DaoFactory.INSTANCE.getLekarDao().dajLekarov().size());
        }
        if(zaznam.getPacient().getId()==0) {
            zaznam.getPacient().setId(DaoFactory.INSTANCE.getPacientDao().dajPacietov().size());
        }
        if(zaznam.getDiagnoza().getId()==0) {
             zaznam.getDiagnoza().setId(DaoFactory.INSTANCE.getDiagnozaDao().dajDiagnozy().size());
        }
        if(zaznam.getLiek().getId()==0) {
             zaznam.getLiek().setId(DaoFactory.INSTANCE.getLiekDao().dajLieky().size());
        }
        
        String sql = "INSERT INTO zaznam (lekar_id, pacient_id, diagnoza_id, liek_id) VALUES (?,?,?,?)";
    ...
    public void vymazZaznam(Zaznam zaznam) {
        String sql = "DELETE FROM zaznam WHERE lekar_id=? AND pacient_id=? AND diagnoza_id=? AND liek_id=? LIMIT 1";
        jdbcTmplate.update(sql, zaznam.getLekar().getId(), zaznam.getPacient().getId(), 
                                zaznam.getDiagnoza().getId(), zaznam.getLiek().getId());
    }
